...
//Adafruit IO Feeds Expoer (to send data to Adafruit.IO)
...
unsigned long lastMQTTSetupTime = 0; // Sets the time when MQTTSetup is called recently. 
...
void CurrentSerial() {  //Shows current ColorMode and BrightMode
...
void setup() {  //The function that runs when the program starts
  ...
  MQTTSetup();  //Connects to MQTT
  HTTPSetup();  //Sets HTTP Server
  PixelSetup();  //Sets NeoPixel
  CurrentSerial();  //Shows current ColorMode and BrightMode
  Serial.println();
}
...
void handleBrightAdafruit(AdafruitIO_Data *data) {
...
void handlemqttAdafruit(AdafruitIO_Data *data) {
